...
wire ar_hs;
// *** Registers ***
reg [31:0] reg0; // imageInput
reg [31:0] reg1; // weightInput
reg [31:0] reg2; // decider
reg [31:0] reg3; // enables and controls

reg [31:0] reg4; // container for outputResult
// project-specific wires
wire [31:0] outputResult;
wire clkEn = reg3[0];
wire imEn = reg3[1];
wire wgEn = reg3[2];
// WARNING: set clkEn to 0 BEFORE changing imEn or wgEn to 0 if imEn & wgEn == 0
...
begin
    ...
    begin
        reg4[31:0] <= (s_axi_wdata[31:0] & wmask) | (outputResult[31:0] & ~wmask);
    end
end    

convmult convmult_8x8_instance(
    .imageInput(reg0),
    .imageInputEnable(imEn),
    .weightInput(reg1),
    .weightInputEnable(wgEn),
    .decider(reg2),
    .processedOutput(outputResult),
    .masterClock(clkEn & (aclk | imEn | wgEn)), // === imEn | wgEn ? clkEn : aclk & clkEn
    // This means manual clock (from reg3) during data input phase, and aclk clock during data process phase
    .masterReset(aresetn)
);
endmodule